/*********************************
   ...
   Adapted from public domain Rijndael implementation by:
         Vincent Rijmen vincent.rijmen@esat.kuleuven.ac.be
	 Antoon Bosselaers antoon.bosselaers@esat.kuleuven.ac.be
	 Paulo Barreto paulo.barreto@terra.com.br

   OpenSSL's Rijndael cipher impl is also adapted from the same original Rijndael.
   The rijndael*() functions are unmodified. The aes_* CBC impl is ours
*******************************/
...
void aes_cbc_decrypt(aes_ctx *ctx, unsigned char *cipher, unsigned char *plain) {
    rijndaelDecrypt(ctx->rk, ctx->nrounds, cipher, plain);
    *(unsigned int *)plain ^= *(unsigned int *)ctx->iv;
    *(unsigned int *)(plain + 4) ^= *(unsigned int *)(ctx->iv + 4);
    *(unsigned int *)(plain + 8) ^= *(unsigned int *)(ctx->iv + 8);
    *(unsigned int *)(plain + 12)^= *(unsigned int *)(ctx->iv + 12);
    memcpy(ctx->iv, cipher, AES_BLOCK_SIZE);
}

void aes_cbc_encrypt(aes_ctx *ctx, unsigned char *plain, unsigned char *cipher) {
    *(unsigned int *)ctx->iv ^= *(unsigned int *)plain;
    *(unsigned int *)(ctx->iv + 4) ^= *(unsigned int *)(plain + 4);
    *(unsigned int *)(ctx->iv + 8) ^= *(unsigned int *)(plain + 8);
    *(unsigned int *)(ctx->iv + 12)^= *(unsigned int *)(plain + 12);
    rijndaelEncrypt(ctx->rk, ctx->nrounds, ctx->iv, cipher);
    memcpy(ctx->iv, cipher, AES_BLOCK_SIZE);
}
...
aes_ctx *aes_key_schedule(unsigned char* key, int keylenbits, unsigned char *iv, int mode) {
    ...
    ctx->rk = CRYPTO_MALLOC(sizeof(unsigned long) * RKLENGTH(keylenbits));
    if (mode == CBC_ENCRYPT) 
        ctx->nrounds = rijndaelSetupEncrypt(ctx->rk, key, keylenbits);
    else
        ctx->nrounds = rijndaelSetupDecrypt(ctx->rk, key, keylenbits);
    ctx->iv = CRYPTO_MALLOC(AES_BLOCK_SIZE);
...
int main() {
    aes_ctx *ctx;
    unsigned char cipher[16], *plain, iv[16], key[16];
    int i;
    ...
    cipher[15] = 0x5e;
    printf("1...\n");
    ctx = aes_key_schedule(key, 128, iv, CBC_DECRYPT);
    printf("2...\n");
    plain = malloc(sizeof(unsigned char) * 16);
    printf("3...\n");
    aes_cbc_decrypt(ctx, cipher, &plain);
    printf("4...\n");
    for (i = 0; i < 16; i++)
        printf("%02x ", plain[i]);
}
